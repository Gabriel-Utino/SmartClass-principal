...
app.post('/notas_faltas', (req, res) => {
  ...
  connection.query(
    'INSERT INTO notas_faltas (id_aluno, id_disciplina, n1, AI, AP, faltas, periodo_letivo) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [
      newNota.id_aluno,
...
app.put('/notas_faltas/:id_notas_faltas', (req, res) => {
  ...
  if (index !== -1) {
    connection.query(
      'UPDATE notas_faltas SET id_aluno=?, id_disciplina=?, n1=?, AI=?, AP=?, faltas=?, periodo_letivo=? WHERE id_notas_faltas=?',
      [
      ...
      err => {
        if (err) {
          ...
          notas_faltas[index] = { ...notas_faltas[index], ...updatedNota }
          res.json(notas_faltas[index])
        }
      }
    )
  } else {
...
app.delete('/notas_faltas/:id_notas_faltas', (req, res) => {
  const id_notas_faltas = parseInt(req.params.id_notas_faltas)
  const index = notas_faltas.findIndex(nota => nota.id_notas_faltas === id_notas_faltas)
  if (index !== -1) {
    connection.query('DELETE FROM Notas WHERE id_notas_faltas=?', [id_notas_faltas], err => {
      if (err) {
        console.error('Notasテーブル - MySQLからのデータ削除エラー: ' + err)
        res.status(500).json({ message: '削除できませんでした' })
      } else {
        const removedNota = notas_faltas.splice(index, 1)
